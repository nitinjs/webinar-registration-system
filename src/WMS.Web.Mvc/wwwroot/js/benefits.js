...
function loadBenefits() {
    abp.services.app.benefits.listAll({ id: webinarId }).done(function (result) {
        ...
        $.each(result, function (key, item) {
            ...
            html += '<td><a href="javascript:void(0)" onclick="return getBenefitbyID(' + item.id + ')"><i class="fas fa-pencil-alt"></i></a> | <a href="javascript:void(0)" style="color:red" onclick="DeleleBenefit(' + item.id + ')"><i class="fas fa-trash"></i></a></td>';
            html += '</tr>';
        });
        if (result.length == 0) {
            html += '<tr>';

...
function loadSpeakerProfiles() {
    abp.services.app.speakerProfiles.listAll({ id: webinarId }).done(function (result) {
        ...
        $.each(result, function (key, item) {
            ...
            html += '<td><a href="javascript:void(0)" onclick="return getSpeakerProfilebyID(' + item.id + ')"><i class="fa fa-pencil"></i></a> | <a href="javascript:void(0)" style="color:red" onclick="DeleleSpeakerProfile(' + item.id + ')"><i class="fa fa-trash"></i></a></td>';
            html += '</tr>';
        });
        if (result.length == 0) {
            html += '<tr>';
